...
# === CLEAN DATA ===
df.columns = df.columns.str.replace(r'\.\d+$', '', regex=True)  # hapus duplikat .1, .2, dst
...
with st.sidebar:
    ...
    filter_kb = st.selectbox("Kelompok Besar", ["(All)"] + sorted(df["Kelompok Besar"].unique()))
    filter_ksnp = st.selectbox("Kelompok Sedang / Nama PJK", ["(All)"] + sorted(df["Kelompok Sedang / Nama PJK"].unique()))
...
# === CHARTS ===
if perspektif == "PJK" or perspektif == "Panglima":
...
if perspektif == "PJK":
    ...
    chart_status = alt.Chart(status_tugas_df).mark_bar().encode(
        ...
        color=alt.Color("Status:N", scale=alt.Scale(domain=["Completed", "Not Completed"],
                                                    range=["#3498db", "#e74c3c"])),
    ...
    chart_melted = alt.Chart(status_df).mark_bar().encode(
        ...
        color=alt.Color("Status:N", scale=alt.Scale(domain=["Graded", "Ungraded"],
                                                    range=["#3498db", "#e74c3c"])),
